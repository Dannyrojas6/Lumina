...
| 运行页可视化配置 | 已完成 | 已可直接修改刷本次数、模式、智能战斗、连续出击和日志级别 |
| 界面状态识别 | 进行中 | 仍依赖模板匹配，稳定性继续补 |
| 智能战斗 v1 | 进行中 | `battle_mode=main` 下由 `smart_battle.enabled` 控制，仍以保守策略为主，且当前结算后会主动停止 |
| 多设备适配 | 未开始 | 当前不做 |
...
- 不做御主技能智能判断
- 不做普通卡完整智能化
- `tests/` 已是正式验证入口之一，但不要把它当成唯一真相
- 主链路相关改动仍要结合现有脚本、日志、调试截图和实际运行结果判断
