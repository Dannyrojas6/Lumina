下面我直接按 **`PLAN.md`** 的写法给你一版，重点不是空谈，而是让你后面真的能落进项目。
...
1. **定义清晰**
   不是空泛说“像人一样思考”，而是明确成“状态 → 候选动作 → 评分 → 选择”。

2. **适合你现在的项目阶段**
   你现在刚从图像识别转到智能战斗，最怕一开始把系统定义得太玄，最后根本写不出来。

3. **后续很好拆代码**
   这个结构和你现在 FGO 自动化项目的模块化方向是匹配的。
